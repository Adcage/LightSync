# LightSync - iFlow CLI 项目指南

> 📅 最后更新: 2025-11-10  
> 🏗️ 项目状态: 开发中 (Phase 1 进行中)  
> 📊 整体进度: 12% (4/34 核心任务完成)  
> 🎯 当前阶段: Phase 1 - 项目基础搭建 (67% 完成, 4/6 任务完成)

---

## 📖 项目概述

**LightSync** 是一个基于 Tauri 2.0 + React 19 + Rust 构建的轻量级跨设备文件同步工具。项目旨在通过 WebDAV 协议实现高效、安全、可定制的文件同步，同时作为学习现代技术栈的实战项目。

### 核心特性

- 🚀 **轻量高效**: 内存占用 < 50MB，启动时间 < 2秒
- 🔄 **双向同步**: 支持本地与云端的双向文件同步
- 🌐 **WebDAV 协议**: 兼容主流 WebDAV 服务商
- ⚙️ **高度可配置**: 灵活的同步策略和冲突处理
- 🔒 **安全可靠**: 密码加密存储，支持 HTTPS
- 🎨 **现代界面**: NextUI + TailwindCSS，支持深色模式

### 技术栈

**前端技术**:
- React 19.1.0 + TypeScript 5.8.3
- NextUI 2.4.8 (UI 组件库)
- TailwindCSS 3.4.18 (样式框架)
- next-themes 0.4.6 (主题管理)
- framer-motion 12.23.24 (动画库)
- Vite 7.0.4 (构建工具)

**后端技术**:
- Rust (Edition 2021)
- Tauri 2.0 (桌面应用框架)
- tokio 1.x (异步运行时)
- notify 6.x (文件系统监控)
- serde 1.x (序列化/反序列化)
- serde_json 1.x (JSON 处理)
- thiserror 1.0 (错误处理)
- chrono 0.4 (日期时间处理)

**Tauri 插件**:
- tauri-plugin-store 2.x (配置存储)
- tauri-plugin-sql 2.x (SQLite 数据库)
- tauri-plugin-fs 2.4.4 (文件系统访问)
- tauri-plugin-opener 2.x (打开文件/URL)

---

## 🏗️ 项目架构

### 目录结构

```
LightSync/
├── src/                          # React 前端源码
│   ├── components/               # React 组件
│   │   ├── TitleBar.tsx         # 自定义标题栏
│   │   ├── ThemeSwitch.tsx      # 主题切换器
│   │   ├── ConfigTest.tsx       # 配置测试组件
│   │   ├── DatabaseTest.tsx     # 数据库测试组件
│   │   └── ErrorBoundary.tsx    # 错误边界
│   ├── hooks/                   # React Hooks
│   │   └── useConfig.ts         # 配置管理 Hook
│   ├── types/                   # TypeScript 类型定义
│   │   └── config.ts            # 配置类型
│   ├── utils/                   # 工具函数
│   │   ├── database.ts          # 数据库操作
│   │   └── store.ts             # 配置存储
│   ├── styles/                  # 样式文件
│   │   └── titlebar.css         # 标题栏样式
│   ├── App.tsx                  # 应用主组件
│   ├── main.tsx                 # 应用入口
│   └── index.css                # 全局样式
│
├── src-tauri/                   # Rust 后端源码
│   ├── src/
│   │   ├── main.rs              # 应用入口
│   │   ├── lib.rs               # 库入口
│   │   ├── error.rs             # 统一错误处理
│   │   ├── config.rs            # 配置管理
│   │   ├── config_watcher.rs    # 配置文件监听
│   │   ├── constants.rs         # 常量定义
│   │   └── database.rs          # 数据库模块
│   ├── migrations/              # 数据库迁移
│   │   └── 001_initial.sql      # 初始数据库架构
│   ├── capabilities/            # Tauri 权限配置
│   ├── icons/                   # 应用图标
│   ├── Cargo.toml               # Rust 依赖配置
│   ├── tauri.conf.json          # Tauri 应用配置
│   └── build.rs                 # 构建脚本
│
├── docs/                        # 项目文档
│   └── md/
│       ├── 项目需求分析.md       # 详细需求规格
│       ├── 项目进度清单.md       # 开发进度跟踪
│       ├── 项目实现步骤.md       # 实施计划
│       └── 自定义标题栏说明.md   # 标题栏实现文档
│
├── package.json                 # Node.js 依赖配置
├── pnpm-lock.yaml              # 依赖锁定文件
├── tsconfig.json               # TypeScript 配置
├── vite.config.ts              # Vite 配置
├── tailwind.config.js          # TailwindCSS 配置
└── README.md                   # 项目说明
```

### 架构设计

```
┌─────────────────────────────────────────────────────────┐
│                    前端层 (React)                        │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │  UI 组件     │  │  Hooks       │  │  工具函数    │  │
│  │  - TitleBar  │  │  - useConfig │  │  - database  │  │
│  │  - Theme     │  │  - useTheme  │  │  - store     │  │
│  └──────────────┘  └──────────────┘  └──────────────┘  │
└─────────────────────────────────────────────────────────┘
                          ↕ Tauri IPC
┌─────────────────────────────────────────────────────────┐
│                   后端层 (Rust)                          │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │  配置管理    │  │  数据库      │  │  文件监控    │  │
│  │  - config    │  │  - SQLite    │  │  - notify    │  │
│  └──────────────┘  └──────────────┘  └──────────────┘  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │  WebDAV      │  │  同步引擎    │  │  错误处理    │  │
│  │  - reqwest   │  │  - tokio     │  │  - thiserror │  │
│  └──────────────┘  └──────────────┘  └──────────────┘  │
└─────────────────────────────────────────────────────────┘
                          ↕
┌─────────────────────────────────────────────────────────┐
│                 存储层 (本地)                            │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │  配置文件    │  │  SQLite DB   │  │  文件系统    │  │
│  │  .config.dat │  │  lightsync.db│  │  本地文件夹  │  │
│  └──────────────┘  └──────────────┘  └──────────────┘  │
└─────────────────────────────────────────────────────────┘
```

---

## 🚀 快速开始

### 环境要求

- **Node.js**: >= 18.0.0
- **pnpm**: >= 8.0.0
- **Rust**: >= 1.70.0
- **操作系统**: Windows 10+, macOS 10.15+, Linux (Ubuntu 20.04+)

### 安装依赖

```bash
# 安装前端依赖
pnpm install

# Rust 依赖会在首次构建时自动安装
```

### 开发命令

```bash
# 启动开发服务器（热重载）- 同时启动前端和后端
pnpm tauri:dev

# 仅启动前端开发服务器（端口 1420）
pnpm dev

# 构建生产版本（TypeScript 编译 + Vite 打包）
pnpm build

# 构建 Tauri 应用（生成可执行文件）
pnpm tauri build

# 运行前端预览
pnpm preview
```

**注意事项**:
- 开发服务器默认运行在 `http://localhost:1420`
- HMR (热模块替换) 端口为 `1421`
- 使用 `pnpm tauri:dev` 可同时启动前后端开发环境

### 代码检查

```bash
# TypeScript 类型检查（不生成文件）
pnpm tsc --noEmit

# Rust 代码检查（Clippy 静态分析）
cd src-tauri
cargo clippy

# Rust 代码格式化
cargo fmt

# 检查代码格式（不修改文件）
cargo fmt --check

# Rust 测试
cargo test

# Rust 构建检查
cargo check
```

---

## 📋 核心功能模块

### 1. 配置管理系统 ✅

**状态**: 已完成  
**文件**: `src-tauri/src/config.rs`, `src/hooks/useConfig.ts`

**功能特性**:
- 统一的配置数据结构 (AppConfig)
- 配置初始化和默认值设置
- 配置 CRUD 操作 (增删改查)
- 配置文件监听和实时同步
- 同步文件夹配置管理
- WebDAV 服务器配置管理

**数据结构**:
```typescript
interface AppConfig {
  version: string;              // 应用版本
  language: string;             // 语言设置 (zh-CN, en-US)
  theme: string;                // 主题 (light, dark, system)
  autoStart: boolean;           // 开机自启动
  minimizeToTray: boolean;      // 最小化到托盘
  syncFolders: SyncFolder[];    // 同步文件夹列表
  webdavServers: WebDavServer[]; // WebDAV 服务器列表
}
```

**可用 Hooks**:
```typescript
// 获取完整配置
const { config, loading, error } = useConfig();

// 获取单个配置值
const { value, loading, error } = useConfigValue<string>('language');

// 更新配置
const { updateConfig, loading, error } = useConfigUpdate();

// 重置配置
const { resetConfig, loading, error } = useConfigReset();
```

**Tauri 命令**:
- `init_config()` - 初始化配置
- `get_config()` - 获取完整配置
- `update_config(config)` - 更新配置
- `get_config_value(key)` - 获取单个配置值
- `set_config_value(key, value)` - 设置单个配置值
- `reset_config()` - 重置为默认配置

### 2. 数据库系统 ✅

**状态**: 已完成  
**文件**: `src-tauri/migrations/001_initial.sql`, `src/utils/database.ts`

**数据表结构**:

**file_metadata** - 文件元数据表
```sql
- id: 主键
- path: 文件路径
- hash: 文件哈希值
- size: 文件大小
- modified_at: 修改时间
- synced_at: 同步时间
- sync_folder_id: 所属同步文件夹
- is_directory: 是否为目录
- status: 同步状态 (synced, pending, conflict, error)
```

**sync_logs** - 同步日志表
```sql
- id: 主键
- sync_folder_id: 所属同步文件夹
- file_path: 文件路径
- action: 操作类型 (upload, download, delete, conflict)
- status: 操作状态 (success, failed, pending)
- error_message: 错误信息
- file_size: 文件大小
- duration_ms: 操作耗时
```

**sync_sessions** - 同步会话表
```sql
- id: 主键
- sync_folder_id: 所属同步文件夹
- status: 会话状态 (running, completed, failed, cancelled)
- started_at: 开始时间
- completed_at: 结束时间
- files_uploaded/downloaded/deleted/conflict: 文件统计
- total_bytes: 总传输字节数
```

**数据库操作**:
```typescript
// 文件元数据 CRUD
await createFileMetadata(metadata);
await getFileMetadata(id);
await updateFileMetadata(id, metadata);
await deleteFileMetadata(id);
await listFileMetadata(syncFolderId);

// 同步日志
await createSyncLog(log);
await listSyncLogs(syncFolderId, limit);

// 同步会话
await createSyncSession(session);
await updateSyncSession(id, session);
await getSyncSession(id);
```

### 3. 错误处理系统 ✅

**状态**: 已完成  
**文件**: `src-tauri/src/error.rs`

**错误类型**:
```rust
pub enum SyncError {
    ConfigError(String),      // 配置错误
    DatabaseError(String),    // 数据库错误
    NetworkError(String),     // 网络错误
    FileSystemError(String),  // 文件系统错误
    WebDavError(String),      // WebDAV 错误
    SerializationError(String), // 序列化错误
    Unknown(String),          // 未知错误
}
```

**特性**:
- 统一的错误类型定义
- 自动错误序列化为 JSON
- 详细的错误信息和上下文
- 支持错误链追踪

### 4. 自定义标题栏 ✅

**状态**: 已完成  
**文件**: `src/components/TitleBar.tsx`

**功能特性**:
- 无边框窗口设计
- 拖拽移动窗口
- 最小化/最大化/关闭按钮
- 最大化状态自动切换图标
- 支持深色/浅色主题
- 优雅的悬停动画效果

**配置**:
```json
// tauri.conf.json
{
  "decorations": false,  // 禁用系统标题栏
  "transparent": false,
  "resizable": true
}
```

### 5. 主题系统 ✅

**状态**: 已完成  
**文件**: `src/components/ThemeSwitch.tsx`

**支持主题**:
- 浅色模式 (Light)
- 深色模式 (Dark)
- 跟随系统 (System)

**实现方式**:
- next-themes 主题管理
- TailwindCSS dark: 类名
- 自动持久化主题选择

---

## 🔄 开发进度

### Phase 1: 项目基础搭建 (Week 1) - 🟡 进行中

**目标**: 完成项目框架搭建，实现基础UI骨架，配置开发环境

**进度**: 67% (4/6 任务完成)

- [x] **Task 1.1**: 项目初始化 ✅
  - 创建 Tauri 项目
  - 配置依赖和构建工具
  - 验证项目可运行

- [x] **Task 1.2**: 统一错误处理系统 ✅
  - 创建 error.rs 模块
  - 定义统一错误类型
  - 实现错误序列化

- [x] **Task 1.3**: 配置管理系统 ✅
  - 创建 config.rs 模块
  - 实现配置 CRUD 操作
  - 创建前端 useConfig Hook
  - 实现配置文件监听

- [x] **Task 1.4**: SQLite 数据库初始化 ✅
  - 注册 SQL 插件
  - 创建数据库迁移文件
  - 实现数据库操作函数
  - 创建测试组件

- [ ] **Task 1.5**: 国际化系统搭建
  - 安装 react-i18next
  - 创建翻译文件
  - 创建语言切换组件

- [ ] **Task 1.6**: 基础UI框架搭建
  - 创建主布局组件
  - 创建路由配置
  - 创建首页组件

### Phase 2-10: 待开始

详细进度请查看 `docs/md/项目进度清单.md`

**总体进度**: 12% (4/34 核心任务完成)

---

## 🛠️ 开发规范

### 代码风格

**TypeScript/React**:
- 使用函数式组件和 Hooks
- 优先使用 TypeScript 类型推导
- 组件使用 PascalCase 命名
- 函数和变量使用 camelCase 命名
- 使用 ES6+ 语法

**Rust**:
- 遵循 Rust 官方代码风格
- 使用 `cargo fmt` 格式化代码
- 使用 `cargo clippy` 检查代码
- 函数和变量使用 snake_case 命名
- 类型使用 PascalCase 命名

### 提交规范

使用语义化提交信息：

```
feat: 新功能
fix: 修复 bug
docs: 文档更新
style: 代码格式调整
refactor: 代码重构
test: 测试相关
chore: 构建/工具相关
```

示例：
```
feat: 实现配置文件监听功能
fix: 修复数据库连接超时问题
docs: 更新 API 文档
```

### 分支管理

```
master          # 主分支，稳定版本
develop         # 开发分支
feature/*       # 功能分支
bugfix/*        # 修复分支
release/*       # 发布分支
```

---

## 📚 重要文档

### 需求与设计

- [项目需求分析](docs/md/项目需求分析.md) - 详细的功能需求和用例分析
- [项目实现步骤](docs/md/项目实现步骤.md) - 分阶段实施计划
- [项目进度清单](docs/md/项目进度清单.md) - 开发进度跟踪

### 技术文档

- [自定义标题栏说明](docs/md/自定义标题栏说明.md) - 标题栏实现细节
- [常量管理最佳实践](docs/md/常量管理最佳实践.md) - 常量定义规范
- [SQLite使用必要性分析](docs/md/SQLite使用必要性分析.md) - 数据库选型分析

### 完成报告

- [Task1.2完成报告](docs/md/Task1.2完成报告.md) - 错误处理系统
- [Task1.3完成报告](docs/md/Task1.3完成报告.md) - 配置管理系统
- [Task1.4完成报告](docs/md/Task1.4完成报告.md) - 数据库系统

---

## 🔧 常见问题

### 构建问题

**Q: Rust 编译失败，提示找不到依赖**  
A: 尝试清理缓存并重新构建：
```bash
cd src-tauri
cargo clean
cargo build
```

**Q: 前端开发服务器启动失败**  
A: 检查端口 1420 是否被占用，或删除 `node_modules` 重新安装：
```bash
rm -rf node_modules
pnpm install
```

### 运行问题

**Q: 应用启动后窗口无法拖动**  
A: 确保 TitleBar 组件正确设置了 `data-tauri-drag-region` 属性

**Q: 配置文件丢失或损坏**  
A: 使用 `reset_config()` 命令重置为默认配置

### 开发问题

**Q: 如何调试 Rust 后端代码？**  
A: 在 `src-tauri/src/lib.rs` 中添加日志：
```rust
println!("Debug: {:?}", variable);
```

**Q: 如何查看数据库内容？**  
A: 使用 SQLite 客户端连接到 `~/.local/share/com.lightsync/lightsync.db`

---

## 📊 性能指标

### 目标性能

| 指标 | 目标值 | 当前状态 |
|------|--------|----------|
| 启动时间 | < 2秒 | 🟡 待测试 |
| 内存占用 (空闲) | < 30MB | 🟡 待测试 |
| 内存占用 (同步) | < 50MB | 🟡 待测试 |
| CPU 使用 (空闲) | < 1% | 🟡 待测试 |
| CPU 使用 (同步) | < 10% | 🟡 待测试 |

### 容量限制

| 项目 | 最大支持 |
|------|----------|
| WebDAV 服务器 | 10 个 |
| 同步文件夹 | 100 个 |
| 单文件夹文件数 | 10万 个 |
| 单文件大小 | 10 GB |
| 总同步数据量 | 1 TB |

---

## 🤝 贡献指南

### 参与开发

1. Fork 项目仓库
2. 创建功能分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'feat: Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 创建 Pull Request

### 报告问题

在 GitHub Issues 中提交问题时，请包含：
- 操作系统和版本
- 应用版本
- 详细的问题描述
- 复现步骤
- 错误日志（如有）

---

## 📄 许可证

本项目采用 MIT 许可证 - 详见 [LICENSE](LICENSE) 文件

---

## 🙏 致谢

### 技术栈

- [Tauri](https://tauri.app/) - 跨平台桌面应用框架
- [React](https://react.dev/) - UI 框架
- [NextUI](https://nextui.org/) - React UI 组件库
- [TailwindCSS](https://tailwindcss.com/) - CSS 框架
- [Rust](https://www.rust-lang.org/) - 系统编程语言

### 参考项目

- [Syncthing](https://syncthing.net/) - 文件同步算法参考
- [rclone](https://rclone.org/) - WebDAV 实现参考

---

## 📞 联系方式

- **项目仓库**: [github.com/Adcage/LightSync](https://github.com/Adcage/LightSync)
- **问题反馈**: [GitHub Issues](https://github.com/Adcage/LightSync/issues)

---

**最后更新**: 2025-11-10  
**维护者**: LightSync 开发团队