        ...
        <CardBody>
          <div className='space-y-4'>
            ...
            <div className='flex flex-wrap gap-2'>
              <Button color='primary' onClick={testInsertFileMetadata} isLoading={loading}>
                插入文件元数据
              </Button>
              <Button color='primary' onClick={testQueryFileMetadata} isLoading={loading}>
                查询文件元数据
              </Button>
              <Button color='secondary' onClick={testInsertSyncLog} isLoading={loading}>
                插入同步日志
              </Button>
              <Button color='secondary' onClick={testQuerySyncLogs} isLoading={loading}>
                查询同步日志
              </Button>
              <Button color='success' onClick={testSyncSession} isLoading={loading}>
                测试同步会话
              </Button>
              <Button color='warning' onClick={testCleanupLogs} isLoading={loading}>
                清理旧日志
              </Button>
              <Button color='default' onClick={testDatabaseStats} isLoading={loading}>
                数据库统计
              </Button>
            </div>
            ...
            {result && (
              <div>
                ...
                <Code className='w-full whitespace-pre-wrap p-4'>{result}</Code>
              </div>
            )}
            ...
            {stats && (
              <div>
                <h3 className='mb-2 text-lg font-semibold'>数据库统计</h3>
            ...
            {fileMetadata.length > 0 && (
              <div>
                <h3 className='mb-2 text-lg font-semibold'>文件元数据 ({fileMetadata.length})</h3>
            ...
            {syncLogs.length > 0 && (
              <div>
                <h3 className='mb-2 text-lg font-semibold'>同步日志 ({syncLogs.length})</h3>
                <div className='space-y-2'>
