...
import { Card, CardBody, CardHeader, Button, Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Chip, Input, Select, SelectItem, DatePicker } from '@nextui-org/react';
...
const LogsPage: React.FC = () => {
  ...
  const getActionChip = (action: string) => {
    switch (action) {
      case 'upload':
        return <Chip color="success" variant="flat" size="sm" startContent={<Upload className="w-3 h-3" />}>
          {t('logs.upload', '上传')}
        </Chip>;
      case 'download':
        return <Chip color="primary" variant="flat" size="sm" startContent={<Download className="w-3 h-3" />}>
          {t('logs.download', '下载')}
        </Chip>;
      case 'delete':
      ...
      case 'conflict':
        return <Chip color="warning" variant="flat" size="sm" startContent={<AlertCircle className="w-3 h-3" />}>
          {t('logs.conflict', '冲突')}
        </Chip>;
      case 'ignored':
        return <Chip color="default" variant="flat" size="sm">
          {t('logs.ignored', '已忽略')}
        </Chip>;
      default:
        return <Chip color="secondary" variant="flat" size="sm">
          {action}
        </Chip>;
    }
  };
  ...
  const getStatusChip = (status: string) => {
    switch (status) {
      case 'success':
        return <Chip color="success" variant="flat" size="sm" startContent={<CheckCircle className="w-3 h-3" />}>
          {t('logs.success', '成功')}
        </Chip>;
      case 'failed':
        return <Chip color="danger" variant="flat" size="sm" startContent={<XCircle className="w-3 h-3" />}>
          {t('logs.failed', '失败')}
        </Chip>;
      case 'resolved':
        return <Chip color="warning" variant="flat" size="sm" startContent={<AlertCircle className="w-3 h-3" />}>
  ...
  const filteredLogs = logs.filter(log => {
    const matchesSearch = log.filePath.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <Card className="mb-6">
        <CardBody>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            >
              <SelectItem key="all" value="all">{t('logs.filterAll', '全部')}</SelectItem>
              <SelectItem key="upload" value="upload">{t('logs.upload', '上传')}</SelectItem>
              <SelectItem key="download" value="download">{t('logs.download', '下载')}</SelectItem>
              <SelectItem key="delete" value="delete">{t('logs.delete', '删除')}</SelectItem>
              <SelectItem key="conflict" value="conflict">{t('logs.conflict', '冲突')}</SelectItem>
              <SelectItem key="ignored" value="ignored">{t('logs.ignored', '已忽略')}</SelectItem>
            </Select>
            ...
            >
              <SelectItem key="all" value="all">{t('logs.filterAll', '全部')}</SelectItem>
              <SelectItem key="success" value="success">{t('logs.success', '成功')}</SelectItem>
              <SelectItem key="failed" value="failed">{t('logs.failed', '失败')}</SelectItem>
              <SelectItem key="resolved" value="resolved">{t('logs.resolved', '已解决')}</SelectItem>
            </Select>
