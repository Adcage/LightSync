...
import { Card, CardBody, CardHeader, Button, Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Chip, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Input, Select, SelectItem, Switch, Textarea } from '@nextui-org/react';
...
const SyncFoldersPage: React.FC = () => {
  ...
  const getStatusChip = (status: string) => {
    switch (status) {
      case 'active':
        return <Chip color="success" variant="flat" size="sm" startContent={<Play className="w-3 h-3" />}>
          {t('folders.active', '同步中')}
        </Chip>;
      case 'paused':
        return <Chip color="warning" variant="flat" size="sm" startContent={<Pause className="w-3 h-3" />}>
      ...
      <ModalContent>
        ...
        <ModalBody>
          <div className="space-y-4">
            ...
            >
              <SelectItem key="1" value="1">坚果云</SelectItem>
              <SelectItem key="2" value="2">NextCloud</SelectItem>
              <SelectItem key="3" value="3">ownCloud</SelectItem>
            </Select>
            ...
            <Select
              ...
              onSelectionChange={(keys) => setFormData({ ...formData, syncInterval: parseInt(String(Array.from(keys)[0])) || 15 })}
            >
              <SelectItem key="1" value="1">{t('folders.realtime', '实时')}</SelectItem>
              <SelectItem key="5" value="5">5 {t('folders.minutes', '分钟')}</SelectItem>
              <SelectItem key="15" value="15">15 {t('folders.minutes', '分钟')}</SelectItem>
              <SelectItem key="30" value="30">30 {t('folders.minutes', '分钟')}</SelectItem>
              <SelectItem key="60" value="60">1 {t('folders.hour', '小时')}</SelectItem>
              <SelectItem key="0" value="0">{t('folders.manual', '仅手动')}</SelectItem>
            </Select>
            ...
            <Textarea
              ...
              onChange={(e) => setFormData({ ...formData, ignorePatterns: (e.target as unknown as HTMLTextAreaElement).value.split('\n').filter((p: string) => p.trim()) })}
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        ...
                        <Switch
                          ...
                          onValueChange={() => handleToggleFolder(item.id)}
                        />
                      </div>
                    </TableCell>
                    <TableCell>
