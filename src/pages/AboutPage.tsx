...
import { Cloud, Github, Globe, Shield, Zap, Users, Package, ExternalLink, Settings } from 'lucide-react'
...
const AboutPage: React.FC = () => {
  ...
  return (
    <div className='mx-auto max-w-4xl p-6'>
      <h1 className='mb-6 text-3xl font-bold text-gray-900 dark:text-gray-100'>{t('about.title', '关于')}</h1>
      ...
      <Card className='mb-6'>
        ...
        <CardBody className='text-center'>
          <div className='mb-4'>
            <p className='text-4xl font-bold text-gray-900 dark:text-gray-100'>{appInfo.version}</p>
            <p className='text-sm text-gray-600 dark:text-gray-400'>
              {t('about.buildNumber', '构建号')}: {appInfo.buildNumber}
            </p>
            <p className='mt-2 text-gray-600 dark:text-gray-400'>{appInfo.description}</p>
          </div>

          <div className='mt-6 grid grid-cols-2 gap-4'>
            <div className='text-center'>
              <p className='mb-1 text-sm text-gray-600 dark:text-gray-400'>{t('about.license', '许可证')}</p>
              <p className='font-medium'>{appInfo.license}</p>
            </div>

            <div className='text-center'>
              <p className='mb-1 text-sm text-gray-600 dark:text-gray-400'>{t('about.author', '作者')}</p>
              <p className='font-medium'>{appInfo.author}</p>
            </div>

            <div className='text-center'>
              <p className='mb-1 text-sm text-gray-600 dark:text-gray-400'>{t('about.releaseDate', '发布日期')}</p>
        ...
        <CardBody>
          <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3'>
            {techStack.map((tech, index) => (
              <div
                key={index}
                className='flex items-center space-x-3 rounded-lg border p-3 transition-colors hover:bg-gray-50 dark:hover:bg-zinc-800'
              >
                <div className='flex-shrink-0'>{tech.icon}</div>
                <div className='flex-1'>
                  <h3 className='font-semibold text-gray-900 dark:text-gray-100'>{tech.name}</h3>
                  <p className='text-sm text-gray-600 dark:text-gray-400'>{tech.version}</p>
                  <p className='mt-1 text-xs text-gray-500 dark:text-gray-400'>{tech.description}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <CardBody>
          <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
            {features.map((feature, index) => (
              <div
                key={index}
                className='flex items-start space-x-4 rounded-lg border p-4 transition-colors hover:bg-gray-50 dark:hover:bg-zinc-800'
              >
                <div className='mt-1 flex-shrink-0'>{feature.icon}</div>
                <div className='flex-1'>
                  <h3 className='mb-2 font-semibold text-gray-900 dark:text-gray-100'>{feature.title}</h3>
      ...
      <Card>
        <CardBody className='text-center'>
          ...
          <p className='text-xs text-gray-500 dark:text-gray-400'>
            {t('about.madeWith', '使用')} {t('about.heart', '❤️')} {t('about.and', '和')} {t('about.coffee', '☕')}{' '}
            {t('about.in', '在')}
          </p>
