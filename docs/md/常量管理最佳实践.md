# 常量管理最佳实践

> 📅 创建日期: 2024-11-07  
> 🎯 目的: 说明 LightSync 项目中的常量管理方案

---

## 📖 为什么需要统一管理常量？

### 问题背景

在开发过程中，如果常量分散在各个文件中，会导致：

```rust
// ❌ 不好的做法 - 常量分散在各个文件中
// config.rs
const CONFIG_FILE: &str = "config.json";

// database.rs  
const DB_FILE: &str = "lightsync.db";

// logger.rs
const LOG_FILE: &str = "lightsync.log";
```

**存在的问题**：
- ❌ 难以查找和修改
- ❌ 可能重复定义
- ❌ 缺乏统一的命名规范
- ❌ 维护成本高

### 解决方案

创建统一的 `constants.rs` 模块：

```rust
// ✅ 好的做法 - 统一管理所有常量
// src-tauri/src/constants.rs

pub const CONFIG_STORE_FILE: &str = "config.json";
pub const DATABASE_FILE: &str = "lightsync.db";
pub const LOG_FILE: &str = "lightsync.log";
```

---

## 🏗️ 常量模块架构

### 文件结构

```
src-tauri/src/
├── constants.rs          # 常量定义模块
├── config.rs            # 使用常量
├── database.rs          # 使用常量
└── ...
```

### 常量分类

`constants.rs` 按功能分为以下几个部分：

#### 1. 文件名常量

```rust
pub const CONFIG_STORE_FILE: &str = "config.json";
pub const DATABASE_FILE: &str = "lightsync.db";
pub const LOG_FILE: &str = "lightsync.log";
```

#### 2. 目录名常量

```rust
pub const CACHE_DIR: &str = "cache";
pub const LOG_DIR: &str = "logs";
pub const TEMP_DIR: &str = "temp";
```

#### 3. 配置默认值

```rust
pub const DEFAULT_LANGUAGE: &str = "zh-CN";
pub const DEFAULT_THEME: &str = "system";
pub const DEFAULT_SYNC_INTERVAL: u32 = 30;
```

#### 4. 应用程序信息

```rust
pub const APP_NAME: &str = "LightSync";
pub const APP_VERSION: &str = env!("CARGO_PKG_VERSION");
pub const APP_DESCRIPTION: &str = env!("CARGO_PKG_DESCRIPTION");
```

#### 5. 网络相关常量

```rust
pub const MAX_RETRY_COUNT: u32 = 3;
pub const RETRY_DELAY_MS: u64 = 1000;
pub const MAX_CONCURRENT_UPLOADS: usize = 5;
```

#### 6. 文件大小限制

```rust
pub const SMALL_FILE_THRESHOLD: u64 = 1024 * 1024;      // 1MB
pub const MEDIUM_FILE_THRESHOLD: u64 = 100 * 1024 * 1024; // 100MB
```

#### 7. 同步相关常量

```rust
pub const DEFAULT_IGNORE_PATTERNS: &[&str] = &[
    ".git",
    ".svn",
    "node_modules",
    "*.tmp",
];

pub mod sync_direction {
    pub const BIDIRECTIONAL: &str = "bidirectional";
    pub const UPLOAD_ONLY: &str = "upload-only";
    pub const DOWNLOAD_ONLY: &str = "download-only";
}
```

---

## 💡 使用方法

### 1. 导入常量

在需要使用常量的模块中导入：

```rust
// 导入所有常量
use crate::constants::*;

// 或者只导入需要的常量
use crate::constants::{CONFIG_STORE_FILE, DEFAULT_LANGUAGE};
```

### 2. 使用常量

```rust
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            version: APP_VERSION.to_string(),
            language: DEFAULT_LANGUAGE.to_string(),
            theme: DEFAULT_THEME.to_string(),
            // ...
        }
    }
}
```

### 3. 使用命名空间常量

```rust
use crate::constants::sync_direction;

// 使用时需要加上模块前缀
let direction = sync_direction::BIDIRECTIONAL;
```

---

## ✅ 最佳实践

### 1. 命名规范

- ✅ 使用 `UPPER_SNAKE_CASE` 命名常量
- ✅ 使用描述性的名称
- ✅ 包含单位信息（如 `_MS`, `_SECONDS`）

```rust
// ✅ 好的命名
pub const MAX_RETRY_COUNT: u32 = 3;
pub const RETRY_DELAY_MS: u64 = 1000;

// ❌ 不好的命名
pub const MAX: u32 = 3;
pub const DELAY: u64 = 1000;
```

### 2. 分组组织

使用注释和空行将相关常量分组：

```rust
// ============================================================================
// 网络相关常量
// ============================================================================

/// 最大重试次数
pub const MAX_RETRY_COUNT: u32 = 3;

/// 重试延迟（毫秒）
pub const RETRY_DELAY_MS: u64 = 1000;
```

### 3. 添加文档注释

为每个常量添加清晰的文档注释：

```rust
/// 小文件阈值（1MB）- 使用完整哈希
pub const SMALL_FILE_THRESHOLD: u64 = 1024 * 1024;
```

### 4. 使用命名空间

对于相关的常量组，使用子模块：

```rust
pub mod sync_direction {
    pub const BIDIRECTIONAL: &str = "bidirectional";
    pub const UPLOAD_ONLY: &str = "upload-only";
    pub const DOWNLOAD_ONLY: &str = "download-only";
}
```

### 5. 环境变量

使用 `env!` 宏读取编译时环境变量：

```rust
pub const APP_VERSION: &str = env!("CARGO_PKG_VERSION");
pub const APP_DESCRIPTION: &str = env!("CARGO_PKG_DESCRIPTION");
```

### 6. 处理未使用警告

对于为未来功能预留的常量，在文件顶部添加：

```rust
#![allow(dead_code)]
```

---

## 🔄 迁移步骤

如果需要将现有常量迁移到 `constants.rs`：

### 步骤 1: 创建 constants.rs

```bash
# 创建文件
touch src-tauri/src/constants.rs
```

### 步骤 2: 移动常量定义

将各个文件中的常量移动到 `constants.rs`：

```rust
// Before: config.rs
const CONFIG_FILE: &str = "config.json";

// After: constants.rs
pub const CONFIG_STORE_FILE: &str = "config.json";
```

### 步骤 3: 在 lib.rs 中注册模块

```rust
// src-tauri/src/lib.rs
mod constants;
```

### 步骤 4: 更新导入

在使用常量的文件中添加导入：

```rust
use crate::constants::*;
```

### 步骤 5: 更新常量引用

```rust
// Before
const CONFIG_FILE: &str = "config.json";
let store = app.store(CONFIG_FILE);

// After
use crate::constants::CONFIG_STORE_FILE;
let store = app.store(CONFIG_STORE_FILE);
```

### 步骤 6: 测试编译

```bash
cargo check
```

---

## 📊 效果对比

### Before (分散管理)

```rust
// config.rs
const CONFIG_FILE: &str = "config.json";

// database.rs
const DB_FILE: &str = "lightsync.db";

// logger.rs  
const LOG_FILE: &str = "lightsync.log";
const MAX_LOG_SIZE: u64 = 10 * 1024 * 1024;
```

**问题**：
- 难以查找所有配置文件名
- 需要搜索多个文件
- 可能存在重复或冲突

### After (统一管理)

```rust
// constants.rs
pub const CONFIG_STORE_FILE: &str = "config.json";
pub const DATABASE_FILE: &str = "lightsync.db";
pub const LOG_FILE: &str = "lightsync.log";
pub const LOG_FILE_MAX_SIZE: u64 = 10 * 1024 * 1024;
```

**优势**：
- ✅ 一目了然
- ✅ 易于维护
- ✅ 统一命名
- ✅ 避免冲突

---

## 🎯 实际应用示例

### 示例 1: 配置管理

```rust
// config.rs
use crate::constants::*;

impl Default for AppConfig {
    fn default() -> Self {
        Self {
            version: APP_VERSION.to_string(),
            language: DEFAULT_LANGUAGE.to_string(),
            theme: DEFAULT_THEME.to_string(),
            // ...
        }
    }
}
```

### 示例 2: 数据库初始化

```rust
// database.rs
use crate::constants::DATABASE_FILE;

pub async fn init_database(app: AppHandle) -> Result<()> {
    let db_path = app.path().app_data_dir()?.join(DATABASE_FILE);
    // ...
}
```

### 示例 3: 日志系统

```rust
// logger.rs
use crate::constants::{LOG_FILE, LOG_FILE_MAX_SIZE, LOG_FILE_RETENTION};

pub fn setup_logger() -> Result<()> {
    let logger = FileLogger::new()
        .file_name(LOG_FILE)
        .max_size(LOG_FILE_MAX_SIZE)
        .retention(LOG_FILE_RETENTION);
    // ...
}
```

---

## 🔍 常见问题

### Q1: 所有常量都要放在 constants.rs 吗？

**A:** 不是。根据常量的使用范围决定：
- **全局共享的常量** → `constants.rs`
- **模块私有的常量** → 保留在模块内
- **临时的魔数** → 考虑提取为命名常量

### Q2: 常量太多会导致文件过大吗？

**A:** 可以进一步模块化：

```rust
// constants/mod.rs
pub mod files;
pub mod network;
pub mod database;

pub use files::*;
pub use network::*;
pub use database::*;
```

### Q3: 如何处理环境特定的常量？

**A:** 使用条件编译：

```rust
#[cfg(debug_assertions)]
pub const API_URL: &str = "http://localhost:3000";

#[cfg(not(debug_assertions))]
pub const API_URL: &str = "https://api.example.com";
```

---

## ✅ 检查清单

在添加新常量时，检查：

- [ ] 常量名称清晰、描述性强
- [ ] 使用了正确的命名规范（UPPER_SNAKE_CASE）
- [ ] 添加了文档注释
- [ ] 放在了合适的分组中
- [ ] 类型正确（&str, u32, u64 等）
- [ ] 包含了单位信息（如果适用）

---

## 📚 相关资源

- [Rust 常量文档](https://doc.rust-lang.org/book/ch03-01-variables-and-mutability.html#constants)
- [Rust 命名规范](https://rust-lang.github.io/api-guidelines/naming.html)
- [项目进度清单](./项目进度清单.md)

---

**总结**: 统一管理常量是一个简单但有效的软件工程实践，能够显著提高代码的可维护性和可读性。

